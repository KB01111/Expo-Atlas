...
export const supabaseService = {
    ...
    try {
      const { data, error } = await supabase
        ...
        .insert({
          ...
          execution_time: execution.execution_time_ms || execution.execution_time,
          cost: execution.cost,
  ...
  async getMCPExecutionHistory(agentId?: string, serverId?: string, limit: number = 50) {
    try {
      let query = supabase
        .from('mcp_tool_executions')
        .select(`
      ...
      console.error('Error fetching MCP execution history:', error);
      return [];
    }
  },

  async getMCPUsageAnalytics(timePeriod: string = '7d') {
    try {
      const daysAgo = timePeriod === '7d' ? 7 : timePeriod === '30d' ? 30 : 1;
      const startDate = new Date();
      startDate.setDate(startDate.getDate() - daysAgo);
      ...
      const analytics = {
        ...
        failed_executions: executions?.filter(e => e.status === 'error').length || 0,
        total_cost: executions?.reduce((sum, e) => sum + (Number(e.cost) || 0), 0) || 0,
        most_used_tools: [],
        most_active_servers: []
      };

      // Calculate most used tools
      const toolUsage = executions?.reduce((acc: any, exec) => {
        acc[exec.tool_id] = (acc[exec.tool_id] || 0) + 1;
        return acc;
      }, {}) || {};

      analytics.most_used_tools = Object.entries(toolUsage)
        .map(([tool_id, count]) => ({ tool_id, count: count as number }))
        .sort((a, b) => b.count - a.count)
        .slice(0, 5);

      // Calculate most active servers
      const serverUsage = executions?.reduce((acc: any, exec) => {
        acc[exec.server_id] = (acc[exec.server_id] || 0) + 1;
        return acc;
      }, {}) || {};

      analytics.most_active_servers = Object.entries(serverUsage)
        .map(([server_id, count]) => ({ server_id, count: count as number }))
        .sort((a, b) => b.count - a.count)
        .slice(0, 5);

      return analytics;
    } catch (error) {
      console.error('Error fetching MCP usage analytics:', error);
      return {
        total_executions: 0,
  ...
  // ========================================
  // ENHANCED OPENAI AGENT METHODS
  // ========================================

  async createOpenAIAgent(agent: any) {
    try {
      console.log('Creating OpenAI agent:', agent.id);
      ...
      const { data, error } = await supabase
        ...
        .insert({
          ...
          created_by: agent.created_by || agent.user_id,
      ...
      if (error) {
        console.error('Supabase error creating OpenAI agent:', error);
        throw error;
      }
      
      console.log('OpenAI agent created successfully:', data);
      return data;
    } catch (error) {
      console.error('Error creating OpenAI agent:', error);
      throw error;
  ...
  async getOpenAIAgents(userId?: string) {
    try {
      let query = supabase
        .from('openai_agents')
        .select('*')
